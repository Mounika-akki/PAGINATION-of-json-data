...
display.setAttribute("class", "table-responsive");
...
//creatin previous button
let previous = document.createElement("button");
previous.classList.add("btn", "btn-info","previous");
previous.innerHTML = "previous";
previous.addEventListener( "click" , event => {
    if(currentButtonValue<=0){currentButtonValue = 0}
...
next.classList.add("btn", "btn-info","next");
next.innerHTML = "Next";
next.addEventListener( "click" , event => {
    if(currentButtonValue >= buttonCount-1){currentButtonValue = buttonCount-1}
    else{currentButtonValue +=1;}
    displaydata(rows,currentButtonValue);
});
